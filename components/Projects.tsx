...
function ProjectCard({ project }: { project: typeof PROJECTS[0] }) {
    ...
    return (
        <div className="group rounded-2xl border border-border/50 bg-card text-card-foreground shadow-sm transition-all hover:shadow-md overflow-hidden flex flex-col h-full">
            <div className="p-6 md:p-8 flex flex-col flex-grow cursor-pointer" onClick={() => setIsExpanded(!isExpanded)}>
                <div className="flex items-start justify-between gap-4 mb-4">
                    <div className="space-y-1">
                        <h3 className="text-xl font-bold group-hover:text-primary transition-colors">
                            {project.title}
                        </h3>
                        <p className="text-sm text-muted-foreground font-medium">{project.role} | {project.year}</p>
                    </div>
                </div>

                <div className="mb-6 flex-grow">
                    <div className="inline-block rounded-lg bg-primary/5 px-3 py-1 text-sm text-primary font-medium mb-4">
                        {project.metric}
                    </div>
                ...
                <button
                    className="text-sm font-medium text-primary mt-auto self-start hover:underline flex items-center gap-1"
                    onClick={(e) => {
                        e.stopPropagation();
